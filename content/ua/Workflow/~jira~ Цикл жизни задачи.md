...
1. **Created** - the task is created and not yet assigned to anyone. It lies in the general task list (Backlog).
2. **To Do** - the task is ready to be executed, and an assignee has been assigned to it. It is waiting for its turn.
3. **In Progress** - the task is being executed.
...
7. **In Testing** - the task is undergoing testing.
...
1. **Blocked** - the task is blocked and cannot be executed for some reason, usually when the execution of the task depends on another task that has not yet been completed.
2. **On Hold** - the task is paused. For example, if a more prioritized task arises in the workflow.
