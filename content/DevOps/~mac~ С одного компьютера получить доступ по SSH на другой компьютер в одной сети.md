...
---
# Как настроить SSH-доступ между двумя компьютерами с macOS в локальной сети
...
В этой статье я покажу, как подключиться по SSH с одного компьютера с macOS к другому. Всё делается буквально за несколько минут.
...
~/.ssh/id_ed25519 — приватный ключ (только у тебя, нельзя никому показывать)
~/.ssh/id_ed25519.pub — публичный ключ (именно его копируют на второй Mac)
