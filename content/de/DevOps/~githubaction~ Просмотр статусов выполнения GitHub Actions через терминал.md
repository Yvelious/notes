...
Jedes Mal auf GitHub über die Web-Oberfläche zu gehen, um den Status von GitHub Actions zu verfolgen, ist nicht besonders praktisch — besonders wenn man häufig Pushes macht und sofort das Ergebnis der CI sehen möchte.
...
In diesem Artikel werden wir uns ansehen, wie man den Status von GitHub Actions, Protokolle und den Fortschritt der Ausführung direkt aus der Konsole anzeigen kann.
...
Nach der Installation muss man sich in seinem GitHub-Konto authentifizieren:
```bash
gh auth login
```
...
Man kann nur die Workflows anzeigen, die gerade ausgeführt werden:
```bash
...
Jede Ausführung hat ihre eigene `id`. Diese kann man aus der Ausgabe des Befehls `gh run list` entnehmen.
...
Um die detaillierten Protokolle jedes Schrittes anzuzeigen, wird das Flag `--log` verwendet:
```bash
...
Um die Ausführung einer GitHub Action in Echtzeit zu verfolgen, kann man den Befehl verwenden:
```bash
...
`gh run watch` fragt einfach die GitHub API in einem bestimmten Intervall ab und aktualisiert die Ausgabe. Dies sind keine Push-Benachrichtigungen, sondern eine regelmäßige Statusüberprüfung.
Dieser Befehl ist praktisch, wenn wir einen bestimmten gestarteten Workflow überwachen. Wenn wir jedoch den Status von 5 oder 10 Workflows, die gestartet, im Prozess oder in der Warteschlange sind, überwachen müssen, ist dieser Befehl nicht besonders geeignet. Dafür können wir das Tool `watch` verwenden, das es ermöglicht, jeden Befehl alle N Sekunden auszuführen und die Ausgabe im Terminal zu aktualisieren.
...
**Mit GitHub CLI und den Standard-Terminal-Tools kann man:**
- die Liste der Ausführungen von GitHub Actions ansehen
- nach Status filtern
- Protokolle einsehen
