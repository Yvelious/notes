...
1. **Globale Ebene**: Ausblendung von Dateien für alle Repositories auf dem Computer. Dies wird in einer Konfigurationsdatei eingestellt, die für alle Repositories des Benutzers gilt.
...
3. **Benutzerebene**: Ausblendung von Dateien nur für einen bestimmten Benutzer und dessen Arbeitsverzeichnis im Repository. Diese Einstellungen werden über die Datei `.git/info/exclude` festgelegt, die nicht in die Versionskontrolle einbezogen wird und nur lokal für diesen Benutzer verfügbar ist.
...
Dies ist genau das `.gitignore`, das sich im Stammverzeichnis des Repositories befindet. Darin sollten hauptsächlich nur die Dinge platziert werden, die einen direkten Bezug zum Projekt und seiner Architektur haben. Zum Beispiel haben alle Teilnehmer des Projekts ein Verzeichnis mit lokalen Daten oder Passwörtern, oder alle erstellen an derselben Stelle temporäre Dateien. Wenn diese Regeln für alle Teilnehmer des Projekts gelten, sollten die Ausblendungsregeln in `.gitignore` platziert werden, das zusammen mit dem Repository verteilt wird.

Früher dachte ich, dass jeder seinen eigenen **.gitignore** hat und dieser nicht committet werden sollte, aber es stellt sich heraus, dass er es sollte, damit alle Teilnehmer des Projekts dieselben Dateien und Ordner ignorieren.

### Konfiguration von gitignore auf Benutzerebene

Dieser Ansatz ermöglicht es dem Benutzer, Dateien und Verzeichnisse in Git **ausschließlich für seine lokale Arbeitskopie des Repositories** auszublenden. Dies ist nützlich, wenn temporäre Dateien, die von diesem Benutzer persönlich generiert werden (z. B. Editor-Dateien, lokale Builds, Protokolle usw.), von der Indizierung ausgeschlossen werden sollen, ohne die Einstellungen anderer Projektteilnehmer zu beeinträchtigen.
...
Die Regeln in `.gitignore` können sowohl einfach als auch komplex sein, was eine präzise Konfiguration des Verhaltens von Git ermöglicht.
...
# keine Dateien verarbeiten, deren Namen auf .a enden
*.a
...
In der Datei `.gitignore` sollten relative Pfade angegeben werden. Diese Pfade beziehen sich **auf das Stammverzeichnis Ihres Repositories** (das Verzeichnis, in dem sich der Ordner `.git` befindet).
...
Wenn Sie einen Ordner zu `.gitignore` hinzugefügt haben, aber `git status` weiterhin Änderungen an den Dateien dieses Ordners anzeigt, liegt das daran, dass Git diese Dateien bereits verfolgt (d. h. sie wurden dem Repository hinzugefügt, bevor Sie sie in `.gitignore` aufgenommen haben).
...
#### 1. Das Verfolgen von Dateien, die bereits im Repository sind, einstellen
Verwenden Sie den Befehl `git rm` mit dem Flag `--cached`, um Dateien aus dem Index zu entfernen (d. h. das Verfolgen zu beenden), ohne sie physisch von der Festplatte zu löschen:
...
```
git commit -m "Stopped tracking Ordner"
```
...
In Git gibt es einen Befehl, der es ermöglicht, **das Verfolgen von Änderungen** an einer bereits verfolgten Datei vorübergehend zu beenden.

Wenn Sie diesen Befehl für eine bestimmte Datei verwenden, wird Git "annehmen", dass diese Datei unverändert bleibt, und sie wird nicht in der Liste der geänderten Dateien angezeigt, wenn Sie den Befehl `git status` ausführen, selbst wenn in die Datei lokale Änderungen vorgenommen wurden.
