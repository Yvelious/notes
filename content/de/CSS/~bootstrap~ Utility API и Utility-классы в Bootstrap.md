...
## Was ist das Utility API und Utility-Klassen in Bootstrap
...
`$utilities` ist ein array-ähnliches Objekt in SASS, das Einstellungen zur Generierung von Utility-Klassen enthält. Es enthält eine Liste aller Utilities (Klassen), die in der endgültigen CSS-Datei generiert werden, sowie deren Einstellungen.
Das Utilities API generiert Klassen basierend auf den Daten aus `$utilities`.

In `_utilities.scss` befindet sich das array-ähnliche Objekt `$utilities` mit einer Reihe von Eigenschaften und Daten zur Generierung von Utility-Klassen.
...
|---|---|---|
|`property`|**Erforderlich**|Der Name der Eigenschaft, dies kann eine Zeichenkette oder ein Array von Zeichenfolgen sein (z. B. horizontale Padding oder Margins).|
|`values`|**Erforderlich**|Eine Liste von Werten oder eine Karte, wenn der Klassenname nicht mit dem Wert übereinstimmen soll. Wenn `null` als Schlüssel der Karte verwendet wird, wird sie nicht kompiliert.|
...
|`state`|Optional|Eine Liste von Pseudoklassenvarianten, wie `:hover` oder `:focus`, die für die Utility generiert werden sollen. Der Standardwert ist nicht vorhanden.|
...
**N.B.**
Alle von der API generierten Utility-Klassen enthalten `!important`, um sicherzustellen, dass sie Komponenten und Modifikator-Klassen nach Bedarf überschreiben. Diese Einstellung kann global über die Variable `$enable-important-utilities` (standardmäßig `true`) umgeschaltet werden.
...
Die Reihenfolge ist wichtig, zuerst muss `$utilities` deklariert werden, bevor es erweitert wird, und danach wird `utilities/api` initialisiert.
...
Dazu verwenden wir ebenfalls `map-merge()`, aber mit einer bestehenden Utility sowie `map-get()`, um die benötigten Eigenschaften zu erhalten.
...
- 
Es gibt jedoch die Möglichkeit, die Unterstützung für Responsive zu bestehenden Utilities hinzuzufügen, beispielsweise zu `height` und `width`.
...
Alternativ kann man auch gezielt eine bestimmte Utility-Klasse, wie z.B. `border`, umschreiben.
```scss
$utilities: map-merge(
  $utilities, (
    "border": map-merge(
...
Das ist ebenfalls möglich, wenn einem der Standardname der Klasse nicht gefällt. Dafür ist die Eigenschaft `class` zuständig.
...
Um beliebige Utilities standardmäßig zu entfernen, setzen Sie den Gruppenschlüssel auf `null`. Zum Beispiel, um die Utility width zu entfernen.
