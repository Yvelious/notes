...
Er ermöglicht es, **Elemente basierend auf dem, was sich in ihnen oder in ihrer Nähe befindet, auszuwählen**.

Früher konnten wir in CSS Kind-Elemente auswählen (`.card img {}`), aber wir konnten **den Elternteil nicht stylen, wenn darin etwas Bestimmtes vorhanden war**. `:has()` löst dieses Problem.

Es kann **in einigen Fällen als Ersatz für JS verwendet werden** (zum Beispiel, um den Elternteil zu stylen, wenn ein Kind aktiv ist).
...
In CSS kann `:has()` mit mehreren Bedingungen verwendet werden — dies ist eine leistungsstarke Möglichkeit, die CSS viel "intelligenter" und kontextabhängiger macht. Die Unterstützung für `:has()` ist in den meisten modernen Browsern verfügbar.
