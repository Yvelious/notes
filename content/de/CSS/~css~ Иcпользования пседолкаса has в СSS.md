...
Er ermöglicht es, **Elemente auszuwählen, basierend darauf, was sich innerhalb oder in der Nähe von ihnen befindet**.

Früher konnten wir in CSS Kind-Elemente auswählen (`.card img {}`), aber wir konnten **Eltern nicht stylen, wenn sie etwas Bestimmtes darin hatten**. `:has()` löst dieses Problem.
...
- `selector` — die Bedingung, was sich darin befinden muss.
...
In CSS kann `:has()` mit mehreren Bedingungen verwendet werden — dies ist eine leistungsstarke Möglichkeit, die CSS viel „intelligenter“ und kontextabhängiger macht. Die Unterstützung von `:has()` ist in den meisten modernen Browsern verfügbar.
...
```
Wendet den Stil auf `<article>` an, wenn **entweder `h1` oder `h2`** oder beide vorhanden sind.
...
```
Wendet den Stil auf `<form>` an, wenn sie mindestens ein ungültiges `<input>` enthält.
