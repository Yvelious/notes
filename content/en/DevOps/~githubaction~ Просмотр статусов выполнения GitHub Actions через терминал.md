...
However, often you only need to see the latest runs and only the required fields.
...
### Outputting Only Required Fields
...
This command is convenient when we are monitoring a specific workflow run. But if we need to monitor the status of 5 or 10 workflows that are running, in process, or queued, then this command is not particularly suitable for our goals. For this, we can use the `watch` utility, which allows you to run any command every N seconds and update the output in the terminal.
...
The `-L 5` flag indicates that we are interested in the last 5 workflows.
