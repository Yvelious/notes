...
In this article, we will discuss how to view the status of GitHub Actions, logs, and progress directly from the console.
...
On macOS, the easiest way to install is via Homebrew:
`brew install gh`
...
The terminal will display all the steps of the workflow.
...
The problem is that GitHub CLI does not stream events in real time. It works via polling—that is, it shows the current state at the time of the request.
...
This command is convenient when we are monitoring a specific run in the workflow. But if we need to monitor the status of 5 or 10 workflows that are running, in progress, or queued, then this command is not particularly suitable for our goals. For this, we can use the `watch` utility, which allows you to run any command every N seconds and update the output in the terminal.
...
- shows the last 5 GitHub Action runs in the terminal
