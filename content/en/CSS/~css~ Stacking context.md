...
3. The element has `transform`, `filter`, `perspective`, `clip-path`, `mask`, `contain`, and a number of other properties.
...
It merely allows you to set a `z-index`. But as long as `z-index: auto` → a stacking context is not created, the element participates in the general layering flow of its parent. Only in combination of these two properties, `z-index` and `position: relative`, will a separate stacking context be created; if only `position: relative` is set, then a stacking context will not be created.
...
It is quite likely to elevate it to a separate compositing layer. However, this decision is made by **the browser itself** based on its internal heuristics and available resources (for example, GPU memory).
