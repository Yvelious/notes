...
In CSS, a **stacking context** is a concept that defines the order of overlapping elements on the page along the Z-axis (i.e., which element will be on top of which). Simply put, it is a "container" for elements, within which their **z-index** and display order are managed.

**Features of stacking context**
- Each **stacking context** forms its own "layer system."
- Elements within a single stacking context overlap one another in the order of **z-index** (or natural flow if z-index is not specified).
...
1. When blending effects (`mix-blend-mode` or `filter`) are used within a container and you need to limit their scope to just that container.
2. When you want to ensure that the z-index of descendants does not conflict with external elements.
...
By setting `isolation: isolate`, you create an "isolated" container, and any blending effects inside it will not extend beyond its bounds and affect elements outside this container.

## position: relative ≠ stacking context  

The property `position: relative` **does not create a stacking context**.  
It only allows setting a `z-index`. But while `z-index: auto` → no stacking context is created, the element participates in the general overlapping flow of the parent. Only the combination of these two properties, z-index and position: relative, will create a separate stacking context; if only `position: relative` is set, no stacking context will be created.

## isolation: isolate = stacking context

The **`isolation`** property in CSS controls the **isolation** of an element. When its value is set to **`isolate`**, it creates a **new stacking context** for that element and its descendants. This isolates descendants from the rest of the page.

- `isolation: auto;` (default value)  
    → the element **does not create** a new stacking context by itself (only if it has properties that do — like `position: fixed`, `z-index`, etc.).
...
**Stacking context** → is a logical grouping of elements for managing overlapping (`z-index`).
**Compositing layer** → is a separate GPU layer for rendering.

Browsers use **`compositing layers`** as an internal optimization mechanism. When the browser sees that an element frequently changes or requires complex re-rendering (for example, due to animations, 3D transformations, or, in our case, creating a new overlapping context), it may decide to "move" this element to a separate layer. This allows the browser to redraw and animate only that layer, without affecting the entire page, which significantly improves performance.
...
It is quite likely to elevate it to a separate compositing layer. However, this decision is made by the **browser itself** based on its internal heuristics and available resources (such as GPU memory). 

If your goal is not only to create a **stacking context** but also for **performance optimization**, and you want to suggest to the browser to create a new compositing layer, it is better to use `will-change: transform` or `will-change: opacity`. These are more explicit "hints" for the browser that have fewer side semantic effects than `isolation`.